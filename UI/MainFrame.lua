...
function UI:SetupMainFrame()
    ...
    frame:SetScript("OnDragStop", function(self)
        ...
        if success and point and x and y then
            ...
            addon.Config:DebugPrint("Saving position to profile:", addon.Config:GetCurrentProfileName(), "X:", validX,
                "Y:", validY)
...
function UI:GetUnavailableIcon()
    ...
    if not icon then
        ...
        icon = CreateFrame("Button", "CCRotationUnavailableIcon" .. numUnavailableIconsCreated, UIParent,
            "CCRotationIconTemplate")
        ...
        icon.deadIndicator = icon:CreateTexture("DeadIndicatorUnavailable_" .. numUnavailableIconsCreated, "OVERLAY", nil,
            1)
        icon.deadIndicator:SetTexture("Interface\\TargetingFrame\\UI-TargetingFrame-Skull")
        icon.deadIndicator:SetPoint("TOPRIGHT", icon, "TOPRIGHT", -1, -1)
        icon.deadIndicator:Hide()

        icon.rangeIndicator = icon:CreateTexture("RangeIndicatorUnavailable_" .. numUnavailableIconsCreated, "OVERLAY",
            nil, 1)
        icon.rangeIndicator:SetTexture("Interface\\RaidFrame\\ReadyCheck-NotReady")
                ...
                if isReady or timeLeft < 3 then
                    ...
                    if icon.spellInfo and icon.unit then
                        ...
                        if iconIndex == 1 then
                            ...
                            icon:SetPoint("TOPLEFT", activeUnavailableIcons[iconIndex - 1], "TOPRIGHT",
                                unavailableSpacing, 0)
                        end
...
function UI:ShowPositionDebug()
    ...
    if self.mainFrame.anchor then
        if self.mainFrame.anchor:IsShown() then
            ...
            print("  Anchor actual position: " ..
                tostring(anchorPoint) .. " X=" .. tostring(anchorX) .. ", Y=" .. tostring(anchorY))
        end
    else
        ...
        if addon.CCRotation.cooldownQueue then
            ...
            for i = 1, math.min(3, #addon.CCRotation.cooldownQueue) do
                ...
                if cd then
                    ...
                    print("    Queue " ..
                        i .. ": " .. (spellInfo and spellInfo.name or "Unknown") .. " (" .. cd.spellID .. ")")
                end
            end
        else
